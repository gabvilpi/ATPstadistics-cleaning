...
entrenamiento <- dataset[index,4:59]
test <- dataset[-index,4:59]
nms<-colnames(dataset)[4:59]
frml <- as.formula(paste("ganador~",paste(nms, collapse = "+"), sep = "",paste("", sep = "")))
NN = neuralnet(frml,entrenamiento, hidden= c(as.integer(length(nms)/2),as.integer(length(nms)/4)), linear.output = FALSE)
